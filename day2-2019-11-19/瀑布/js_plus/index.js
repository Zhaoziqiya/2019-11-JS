...
const head = document.querySelector('.head');
    ...
    *    findIndex(callback)
    * 
    * 找到回调函数中return后符合条件的索引值，找不到为-1 callback(数组每项，索引，all) 
    * 
    * 问题：
...
function minEle(lis){
    ...
    // 返回就3个值   ele:最短的元素，index:最短的索引，min:最短的值
    return {
        ele:lis[index],
        index,
        min
...
function fn (){
    ...
    if(iH + scroll >= min + head.offsetHight){
        console.log('触底了');
        // console.log('1');
        render();
    }
    // console.log(iH + scroll);
    // console.dir(min+head.offsetHight)
}